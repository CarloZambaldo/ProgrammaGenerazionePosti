...
Programma che crea a random i posti,
   input necessari: - Numero studenti
    ...
    funzioni aggiuntive: - Crea un documento .txt per salvare una disposizione
                         - Per salvare la disposizione bisogna immettere un nome utente e la password
...
# add a slider http://www.tutorialspoint.com/python/tk_scrollbar.htm
versione = "3.6.0"
datagg = "26 GEN 2016"
...
class posti:
    ...
    def begin(self):
        ...
        self.scale = Scale(self.menu, from_=0, to=10000, orient=HORIZONTAL)
        self.scale.set(100)
    ...
    def nuofile(self, f, bpf, numero):
        ...
        if (((NU!="")and((PSW=="CONFERMO")or(PSW=="Confermo")or(PSW=="ok")))):
            try:
                ...
                out_file.write("Created by "+NU+" on: "+time.strftime("%d/%m/%Y")+" at "+time.strftime("%H:%M:%S"))
                out_file.close()
