...
class posti:
    ...
    def begin(self):
        ...
        self.e1 = Entry(self.menu, textvariable=1, width = 10, justify=CENTER)
        ...
        Label(self.menu, text="Advanced: ", fg = 'black',font=('times', 15)).grid(row=5, column=0,stick = E)
        Button(self.menu, text = 'Condizione', state=DISABLED, fg = 'black', relief="groove",font=('times', 15),command = lambda: self.ADV()).grid(row = 5, column = 1, stick = N)
        
        # FILE GIA' CREATO
        Label(self.menu, text="File posti.txt gia' creato?", fg = 'green',font=('times', 16)).grid(row=4, column=0,stick = E)
        Button(self.menu, text = 'OPEN FILE', fg = 'black', relief="groove",font=('times', 14),command = lambda: self.aprifile()).grid(row = 4, column = 1, stick = N)

        # BOTTONI FONDO
        Button(self.menu, text = 'GENERA', fg = 'blue', relief="groove",font=('times', 17, "bold"),command = lambda: self.avanti()).grid(row = 6, column = 0, stick = N)
        Button(self.menu, text = 'HELP', fg = 'black', relief="groove", font=('times', 14),command = lambda: self.info()).grid(row = 6, column = 1, stick = N)

        # OPZIONE QUANTO MESCOLA
        Label(self.menu, text="Quanto mescolo?", fg = 'black',font=('times', 16)).grid(row=3, column=0,stick = E)
    ...
    def ADV(self):
        ...
        Button(self.ADVANCED, text = 'CANCEL', fg = 'red', relief="groove",font=('times', 13, "bold"),command = lambda: self.ADVANCED.destroy()).grid(row = 0, column = 2, stick = W)
        self.ADVANCED.pack()
